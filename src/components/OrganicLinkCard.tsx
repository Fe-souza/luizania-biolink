...
const OrganicLinkCard = ({
  ...
  const metallicGold =
    "linear-gradient(135deg, #8a6a1f 0%, #d4af37 22%, #fff3b0 48%, #f5d76e 58%, #d4af37 78%, #7a5d18 100%)";

  const getVariantStyles = () => {
    switch (variant) {
      case "featured":
        return {
          bg: "bg-gradient-to-br from-amber-200/95 via-yellow-100/90 to-amber-100/85",
      ...
      case "info":
        return {
          ...
          glow: "shadow-[0_20px_50px_-15px_rgba(212,175,55,0.3)]",
      ...
      default:
        return {
          ...
          glow: "shadow-[0_20px_50px_-15px_rgba(212,175,55,0.25)]",
          iconBg: "text-amber-400",
          iconStyle: {
            background:
              "linear-gradient(135deg, #1a1a1a 0%, #0a0a0a 50%, #1a1a1a 100%)",
    ...
    <>
      ...
      {icon && (
        ...
        >
          ...
          />
          <div
            className={`relative w-full h-full flex items-center justify-center rounded-2xl ring-2 ring-amber-300/40 group-hover:ring-amber-500/60 transition-all duration-300 ${styles.iconBg}`}
            style={{
              ...styles.iconStyle,
              boxShadow:
                "inset 0 1px 0 rgba(255,243,176,0.55), inset 0 -1px 0 rgba(0,0,0,0.35)",
            }}
          >
            {icon}
          </div>
